...
contract('SDC', (accounts) => {
    ...
    it('several verifications with changing nonce (approveSig + getNonce) test', async function() {
        ...
        for (let i = 0; i < repeats; i++) {
            let sig = await getSignature(sdc, addr1, addr2)
    ...
    it('approveSig pause test', async function() {
        let sdcPause = await SDC.new({from: accounts[0]})
        let value = 1337
        let sig = await getSignature(sdcPause, accounts[0], accounts[1])
        await sdcPause.approveSig.sendTransaction(value, accounts[0], accounts[1], sig)

        sig = await getSignature(sdcPause, accounts[0], accounts[1])
